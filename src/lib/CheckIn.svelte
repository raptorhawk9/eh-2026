...
<div class="align-center flex w-full flex-row items-center border border-white">
	{#if !isActive}
	...
	{:else}
		<div class="w-full p-4 flex flex-col gap-4">
            ...
            <div class="flex flex-row justify-center align-middle text-2xl">{Math.trunc(time / 60)}:{time % 60}</div>
            <button class="w-full bg-green-600 border border-white p-1 hover:cursor-pointer" onclick={reset}>I'm Safe</button>
		</div>
	{/if}
</div>
